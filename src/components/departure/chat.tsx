...
export const Chat: React.FC = () => {
  ...
  const getPhotoUrl = (message: Message): string => {
    if (message.photoUrl) {
      // If photoUrl is already an absolute URL, return as-is, otherwise prefix with backend host
      if (message.photoUrl.startsWith("http://") || message.photoUrl.startsWith("https://")) {
        return message.photoUrl;
      }
      return `https://api.backconcursemos.com${message.photoUrl}`;
    }
                ...
                >
                  ...
                  >
                    {isOwnMessage(message.fromUserId) ? (
                      <img
                        src={profile?.photoUrl ?? "/images/Logos/Logo-login.png"}
