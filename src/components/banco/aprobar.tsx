...
export const Aproved: React.FC<AprovedProps> = ({
  question,
  token,
  setUpdate,
}) => {
  ...
  const confirmBlock = (): void => {
    Swal.fire({
      title: "¿Estás Seguro?",
      text: `¿Deseas aprobar la pregunta "${
  ...
  const handleApprove = (): void => {
    // Aquí implementarás la llamada a la API cuando tengas el backend
    console.log("Aprobando pregunta ID:", question.id);
    console.log("Token disponible:", token);

    // Por ahora solo actualizamos el estado para simular
    setUpdate((prev) => !prev);

    Swal.fire({
      title: "¡Pregunta aprobada!",
      text: `La pregunta ha sido aprobada exitosamente.`,
      icon: "success",
      confirmButtonColor: "#10b981",
